...
export async function getDashboardStats() {
  const [totalJobs, totalCategories, byStatusRaw, byCategoryRaw, immediateJoinerCount, companiesRaw] =
    await Promise.all([
      prisma.job.count(),
  ...
  const totalCompanies = companiesRaw.filter((row) => row.company).length;
